...
(ert-deftest agent-shell--tool-call-content-text-test ()
  "Test `agent-shell--tool-call-content-text'.

Tool call CONTENT items wrap a content block under `content'.  Each block
is rendered via `agent-shell--content-block-to-markdown' so image,
resource_link, and embedded resource blocks render richly rather than
being dropped when they carry no `text'."
  ;; A wrapped text block -> its text.
  (should (equal (agent-shell--tool-call-content-text
                  [((type . "content")
                    (content . ((type . "text") (text . "hello"))))])
                 "hello"))

  ;; A wrapped image block -> a markdown image, not a silent drop.
  (should (equal (agent-shell--tool-call-content-text
                  [((type . "content")
                    (content . ((type . "image")
                                (mimeType . "image/png")
  ...
  ;; A wrapped resource_link -> a clickable markdown link.
  (should (equal (agent-shell--tool-call-content-text
                  [((type . "content")
  ...
  ;; A bare text item (no `content' wrapper) still contributes its text.
  (should (equal (agent-shell--tool-call-content-text
                  [((type . "text") (text . "bare"))])
                 "bare"))

  ;; Multiple items are joined with blank lines.
  (should (equal (agent-shell--tool-call-content-text
                  [((type . "content")
...
prompt -- unlabeled, not as an error fragment."
  (let ((state (list (cons :chunked-group-count 0)
                     ;; Turn 1 ended with an agent message; the out-of-turn
                     ;; burst is kept separate by its dedicated namespace, not
                     ;; by group/create-new state.
                     (cons :last-entry-type "agent_message_chunk")
    ...
    (cl-letf (((symbol-function 'agent-shell--emit-event) #'ignore)
      ...
      (should above-flag)
      ;; Separated from the previous turn by its dedicated namespace.
...
(ert-deftest agent-shell--on-notification-out-of-turn-thought-is-separate-test ()
  ...
  (let ((state (list (cons :chunked-group-count 0)
    ...
    (cl-letf (((symbol-function 'agent-shell--append-transcript) #'ignore)
              ((symbol-function 'agent-shell--update-fragment)
               (lambda (&rest args)
                 ;; The thought branch also refreshes the activity-group
                 ;; header; capture the thought fragment itself (the call
                 ;; carrying the body).
...
(ert-deftest agent-shell--on-notification-out-of-turn-user-message-reports-anomaly-test ()
  ...
  (let ((state (list (cons :active-requests nil)
                     (cons :last-entry-type nil)
                     (cons :last-activity-time nil)
                     (cons :agent-config '((:mode-line-name . "Test agent")))))
        (captured nil)
        (above-flag nil))
    (cl-letf (((symbol-function 'agent-shell--update-text)
               (lambda (&rest _args)
                 (ert-fail "Out-of-turn user message inserted a prompt")))
              ((symbol-function 'agent-shell--update-fragment)
               (lambda (&rest args)
                 (setq captured args)
                 (setq above-flag (bound-and-true-p agent-shell--render-above-prompt)))))
      ...
      (should (equal (plist-get captured :block-id)
                     "out-of-turn-user-message-chunk"))
      (should (string-search "Test agent" (plist-get captured :body)))
      (should above-flag))))
...
(ert-deftest agent-shell--on-notification-tool-call-display-terminal-exit-fails-status-test ()
  "Non-zero terminal exits stay failed through completion and transcript."
  (let ((state (list (cons :buffer (current-buffer))
                     (cons :tool-calls nil)
                     (cons :activity-group-count 0)
                     (cons :expanded-activity-group nil)
    ...
    (cl-letf (((symbol-function 'agent-shell--active-requests-p)
              ...
              ((symbol-function 'agent-shell--append-transcript)
               (lambda (&rest args)
                 (push (plist-get args :text) transcript-entries)))
              ((symbol-function 'agent-shell--cancel-idle-timer)
               #'ignore)
              ((symbol-function 'agent-shell--emit-event)
               #'ignore)
      ...
      (should (equal (map-nested-elt state '(:tool-calls "tc1" :status))
                     "failed"))
      ;; A later protocol-level completion must not erase the terminal failure.
      (agent-shell--on-notification
       :state state
       :acp-notification
       '((method . "session/update")
      ...
      (should (equal (map-nested-elt state '(:tool-calls "tc1" :status))
                     "failed"))
      (should (= (length transcript-entries) 1))
      (should (string-match-p
               (regexp-quote "### Tool Call [failed]")
               (car transcript-entries))))))
...
and avoid inserting paragraph breaks between chunks of one prompt."
  (let ((file (make-temp-file "agent-shell-transcript"))
        (agent-shell--transcript-file nil)
        ;; Replayed into a plain buffer, which holds no live prompt to
        ;; render above: what is under test is the transcript the replay
        ;; writes, not where in a shell it lands.
...
When late fragments render above the live prompt, viewport reconstruction needs
that tail in addition to the normal prompt/response pair."
  (with-temp-buffer
    (let* ((tail "\n\nAfter turn\n\n")
           (tail-start nil)
           (tail-end nil)
      ...
      (insert "prompt> ")
      (setq prompt-end (point-marker))
      (add-text-properties tail-start tail-end
                           '(agent-shell-after-turn t))
      (setq-local comint-last-input-start (copy-marker (point-min))
                  comint-last-prompt (cons prompt-start prompt-end))
      (goto-char prompt-start)
      (cl-letf (((symbol-function 'agent-shell--shell-buffer)
                 (lambda (&rest _) (current-buffer)))
                ((symbol-function 'shell-maker--command-and-response-at-point)
                 (lambda (&rest _) '("prompt" . "response"))))
        (setq interaction (agent-shell-interaction-at-point))
        (should (equal (map-elt interaction :prompt) "prompt"))
        (should (equal (map-elt interaction :response) "response"))
        (should (equal (map-elt interaction :after-turn) "\n\nAfter turn"))))))

(ert-deftest agent-shell-after-turn-tail-preserves-fragments-test ()
  "Recover late fragments without applying clipboard transformations."
  (with-temp-buffer
    (let ((inhibit-read-only t))
      (insert "Response\n\n")
      (let ((start (point)))
        (agent-shell-ui-update-fragment
         (agent-shell-ui-make-fragment-model
          :namespace-id 1 :block-id "late" :label-left "Command"
          :body "Late output" :group-id "activity" :group-label "Ran a command")
         :expanded nil)
        (add-text-properties start (point-max) '(agent-shell-after-turn t)))
      (goto-char (point-max))
      (let ((start (point-marker)))
        (insert "Claude> ")
        (setq-local comint-last-prompt (cons start (point-marker))))
      (let* ((case-fold-search nil)
             (tail (agent-shell--after-turn-tail-before-live-prompt))
             (position (string-match "Command" tail))
             (state (get-text-property position 'agent-shell-ui-state tail)))
        (should (equal (map-elt state :qualified-id) "1-late"))
        (should (equal (map-elt state :group-id) "1-activity"))
        (should (map-elt state :collapsed))
        (should (equal (get-text-property position 'line-prefix tail) "  "))
        (should (string-match-p "▶ Command" tail))))))

(ert-deftest agent-shell-after-turn-reconstruction-does-not-duplicate-test ()
  "Real history extraction already includes late output before the prompt."
  (dolist (options '((nil nil) (nil t) (t nil) (t t)))
    (let ((expanded (car options))
          (persistent (cadr options)))
      (with-temp-buffer
        (comint-mode)
        (setq-local major-mode 'agent-shell-mode
                    comint-prompt-regexp "Claude> "
        ...
        (let ((inhibit-read-only t))
          ...
          (let ((start (point)))
            (agent-shell-ui-update-fragment
             (agent-shell-ui-make-fragment-model
              :namespace-id 1 :block-id "late" :label-left "Late command"
              :body "Late output" :group-id "activity" :group-label "Ran a command"
          ...
          (let ((shell-buffer (current-buffer)))
            (with-temp-buffer
              (cl-letf (((symbol-function 'agent-shell--shell-buffer)
                        ...
                        ((symbol-function 'agent-shell-viewport--position)
                         (lambda (&rest _) nil))
                        ((symbol-function 'agent-shell-viewport--update-header) #'ignore))
                (agent-shell-viewport-view-mode)
                (dotimes (_ 2)
                  (let* ((interaction (agent-shell-viewport-refresh))
                         (response (buffer-string))
                         (position (string-match "Late command" response)))
                    (should-not (and (map-elt interaction :after-turn) t))
                    (should (= 1 (1- (length (split-string response "Late command")))))
                    (should (equal (map-elt (get-text-property position 'agent-shell-ui-state response)
                                            :group-id)
                                   "1-activity"))))))))))))

(ert-deftest agent-shell-interaction-at-point-excludes-latest-after-turn-from-history-test ()
  "A historical interaction must not inherit the latest after-turn tail."
  (with-temp-buffer
    (insert "historical response\n\n")
    (let ((historical-point (copy-marker (point-min)))
          (latest-input-start (copy-marker (point)))
          tail-start prompt-start prompt-end interaction)
      (insert "latest response")
      (setq tail-start (point))
      (insert "\n\nLate tool call\n\n")
      (add-text-properties tail-start (point) '(agent-shell-after-turn t))
      (setq prompt-start (copy-marker (point)))
      (insert "prompt> ")
      (setq prompt-end (point-marker))
      (setq-local comint-last-input-start latest-input-start
                  comint-last-prompt (cons prompt-start prompt-end))
      (goto-char historical-point)
      (cl-letf (((symbol-function 'agent-shell--shell-buffer)
                 (lambda (&rest _) (current-buffer)))
                ((symbol-function 'shell-maker--command-and-response-at-point)
                 (lambda (&rest _) '("old prompt" . "historical response"))))
        (setq interaction (agent-shell-interaction-at-point))
        (should (equal (map-elt interaction :prompt) "old prompt"))
        (should (equal (map-elt interaction :response) "historical response"))
        (should-not (map-elt interaction :after-turn))))))

(ert-deftest agent-shell-interaction-at-point-on-live-prompt-is-latest-test ()
  "Point on the live prompt resolves to the newest interaction.

Nothing is under point there, and shell-maker's backwards fallback
skips the last response when it equals the last output, so a viewport
refresh from the newest page used to show the page before it."
  (let ((shell-buffer (agent-shell-viewport-tests--make-shell-buffer
                       '(("one" . "first") ("two" . "second") ("three" . "third")))))
    (unwind-protect
        (with-current-buffer shell-buffer
          (goto-char (point-max))
          ;; Nothing under point on the live prompt, as in a real shell,
          ;; whose comint field layout this buffer cannot reproduce.
          (cl-letf* ((live-prompt (save-excursion (re-search-backward "Claude> ") (point)))
                     (at-point (symbol-function 'shell-maker--command-and-response-at-point))
                     ((symbol-function 'shell-maker--command-and-response-at-point)
                      (lambda (&rest args)
                        (unless (>= (point) live-prompt)
                          (apply at-point args))))
                     ((symbol-function 'agent-shell--shell-buffer)
                      (lambda (&rest _) shell-buffer))
                     ((symbol-function 'shell-maker-last-output)
                      (lambda () "third")))
            (let ((interaction (agent-shell-interaction-at-point)))
              (should (equal (string-trim (map-elt interaction :prompt)) "three"))
              (should (equal (string-trim (map-elt interaction :response)) "third")))))
      (kill-buffer shell-buffer))))
...
(ert-deftest agent-shell--render-tool-call-fragment-preserves-grouping-test ()
  "The centralized tool-call renderer keeps upstream grouping behavior."
  (let ((state (list (cons :buffer (current-buffer))
                     (cons :request-count 1)
                     (cons :last-entry-type "agent_message_chunk")
                     (cons :activity-group-count 0)
                     ...
                     (cons :tool-calls
                           '(("tc1" . ((:title . "Read")
                                       (:status . "in_progress")
                                       (:kind . "read")))))))
        (agent-shell-activity-group-header-label-function
         #'agent-shell-activity-group-count-label)
        calls)
    (cl-letf (((symbol-function 'agent-shell--update-fragment)
               (lambda (&rest args)
                 (push args calls))))
      (agent-shell--render-tool-call-fragment
       :state state
       :tool-call-id "tc1"))
    (let ((member-call
           (seq-find (lambda (args)
                       (equal (plist-get args :block-id) "tc1"))
                     calls))
          (group-call
           (seq-find (lambda (args)
                       (equal (plist-get args :block-id) "activity-1"))
                     calls)))
      (should (equal (plist-get member-call :group-id) "activity-1"))
      (should (equal (plist-get member-call :group-label)
                     agent-shell--activity-group-label))
      (should (eq (plist-get member-call :group-expanded)
                  (agent-shell--activity-group-initial-expanded-p)))
      (should group-call)
      (should (string-match-p
               "Activity 0/1"
               (substring-no-properties
                (plist-get group-call :label-left)))))))
...
when the member is collapsed, joining both headers on one line."
  (with-temp-buffer
    ...
    (let (;; This buffer holds no live prompt, and rendering above one is
          ;; not what is under test: `agent-shell--update-fragment' would
          ;; only signal (see
          ;; `agent-shell--live-prompt-start-asserts-missing-prompt-test').
          (agent-shell-persistent-prompt-enabled nil)
          (state `((:buffer . ,(current-buffer))
                   (:request-count . 1))))
