...
PROMPT to that turn, `queue' holds it until the turn ends, and nil leaves
the choice to `agent-shell-busy-submit-default-function'.  An explicit
DISPOSITION is how an entry point keeps its own promise whatever that
setting says.

Explicit `steer' and the setting differ in what happens when steering is
not on offer.  Explicit `steer' errors, and never falls back once the
agent has declined, because the user asked to steer and silently doing
something else is worse than saying so.  A nil DISPOSITION stays
forgiving, because the user asked to send.
...
so a caller holding its own copy can drop it.  Neither queueing nor a nil
DISPOSITION calls it: the queue owns that copy now, and what a nil
DISPOSITION runs is a user-supplied function taking the prompt alone."
  (agent-shell--prompt-queue-migrate)
  (cond
   ((eq disposition 'steer)
    (agent-shell--prompt-steer :prompt prompt :on-delivered on-delivered))
   ((not (agent-shell--prompt-submittable-p))
    (if (eq disposition 'queue)
        (agent-shell--prompt-queue-enqueue :prompt prompt)
...
`agent-shell-busy-submit-default-function' decides which it is.  Use
...
Steering is the default here so that redirecting a turn going the wrong
way is what a plain send does, rather than something to know to reach
for.  Set this to `agent-shell-busy-submit-queue' to wait for the turn to
end instead.  Nothing is lost either way: an agent that cannot take a
steered prompt, one that declines it, and a shell waiting on a permission
answer all queue the prompt.
...
Queues PROMPT instead wherever a steer would not really reach the turn:
an agent that never advertised the capability, and a shell blocked on a
permission answer, which no implementation defines a steered prompt for.
So choosing this does not mean an error every turn.
`agent-shell-prompt-steer' is the command that refuses outright.

An agent that takes the request and then declines it queues PROMPT too,
rather than interrupting the turn the way `agent-shell-prompt-steer'
does: the user asked to send a prompt, and the queue is somewhere for it
to go.

Steering is not additive: see `agent-shell-prompt-steer' for what the
agent may drop in order to change course."
  (if (agent-shell--prompt-steerable-p)
      (agent-shell--prompt-steer-or-queue :prompt prompt)
    (agent-shell--prompt-queue-enqueue :prompt prompt)))

(defun agent-shell--busy-submit-steers-p ()
  "Return non-nil when a plain mid-turn submission steers.

Only the two functions this package ships can be read this way.  A custom
`agent-shell-busy-submit-default-function' answers nil, since what it
does is its own business -- callers naming the behaviour to the user have
to say so rather than guess."
  (eq agent-shell-busy-submit-default-function #'agent-shell-busy-submit-steer))
