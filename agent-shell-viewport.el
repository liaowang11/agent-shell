...
`steer', `queue', or nil to follow `agent-shell-prompt-while-busy'.
...
`agent-shell-prompt-while-busy' and `agent-shell-viewport-compose-steer'
selects steering, this selects queue delivery whatever the setting is.
...
selects queue delivery, this selects steering: it hands the draft to the
turn already running (see `agent-shell-prompt-steer'), so the agent can
change course mid-work.
...
in this viewport buffer."
  (let ((steerable
    ...
    (cond
     ...
     ((and (eq agent-shell-prompt-while-busy 'steer) steerable) "steer")
     (t "queue"))))
