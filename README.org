...
In edit mode, =C-c C-c= sends the prompt and switches to view mode when viewport interaction is preferred.  Use =C-u C-c C-c= to send while keeping the compose buffer open, =C-c C-q= to wait behind a running turn or submit when idle, and =C-c C-i= to steer the running turn instead (see [[*Steering a running turn]]).  Use =C-c C-p= to peek at the latest interaction without losing the draft, =C-c C-k= to cancel, and =C-c C-h= for help.
...
Each behavior has a command that always does that one thing, so a binding never depends on a setting:

- =M-x agent-shell-prompt-queue= - queue until the running turn ends, or submit immediately when idle.
- =M-x agent-shell-prompt-steer= - steer the running turn, submit when idle, or error when the agent cannot steer.
- =M-x agent-shell-prompt-send= - follow =agent-shell-busy-submit-default-function= (see below).
- =M-x agent-shell-prompt-queue-steer= - steer a prompt that is already queued.  Also available as =Steer (s)= in the pending prompts block.

Each of the first three has a =-dwim= variant (=agent-shell-prompt-send-dwim=, =agent-shell-prompt-queue-dwim=, =agent-shell-prompt-steer-dwim=) that prefills the region or error at point as context, so the thing you are looking at can redirect work already under way.  With a prefix argument they ask which shell to use.

=agent-shell-busy-submit-default-function= decides what a plain send does while the agent works, everywhere one can be made: the shell prompt (=RET=), =agent-shell-prompt-send=, and the compose buffer's send key.  This fork defaults it to =agent-shell-busy-submit-steer=; set it to =agent-shell-busy-submit-queue= to wait for the turn to end.  A plain send stays lossless whichever way it is set: with no turn running the prompt is submitted, and an agent that cannot take a steered prompt, one that declines it, or a shell waiting on a permission answer all get the prompt queued instead.

=agent-shell-busy-submit-override-function= is the other one, reached without changing the setting: =C-u RET= or =M-RET= at the shell prompt (=agent-shell-submit-override=), and =M-RET= in the compose buffer (=agent-shell-viewport-compose-send-override=).  It defaults to =agent-shell-busy-submit-queue= here, so whichever of the two is not the default is always one keystroke away.  Both accept any function of one argument if neither behavior is what you want.
...
A steered prompt renders where the agent's output paused, under a =Steered prompt= heading, rather than as another shell prompt.  It belongs to the turn it steered, so the turn stays a single interaction and the viewport shows the steer and the response it produced together.

Steering is not additive.  An agent that interrupts may drop the instruction it was working on, so a steer can replace what the agent was already doing rather than adding to it.  Queue instead when the agent should finish first.  This is why the two commands differ on what a declined steer means: =agent-shell-prompt-steer= interrupts the running turn and says so, since you asked to steer and quietly doing something else is worse; a plain send just queues, since you asked to send.

**** Compose buffer

The compose buffer's keys line up with those three commands: =C-c C-c= (=agent-shell-viewport-compose-send=) follows the setting, =C-c C-q= (=agent-shell-viewport-compose-queue=) queues behind a running turn or submits when idle, and =C-c C-i= (=agent-shell-viewport-compose-steer=) steers.  =M-RET= (=agent-shell-viewport-compose-send-override=) sends through =agent-shell-busy-submit-override-function= instead, the compose-buffer counterpart of =C-u RET= at the shell prompt.  All of them leave the compose buffer the same way after delivery, following =agent-shell-viewport-dismiss-on-send= and =agent-shell-prefer-viewport-interaction=; with a prefix argument (=C-u=) the buffer stays open, cleared, for the next prompt.

A compose buffer carries the disposition of whichever command opened it, and that outranks both the setting and =M-RET=.  A draft started by =agent-shell-prompt-queue-dwim= still queues while the setting says steer, because a draft outlives the command that started it.  While a turn is running, the edit-mode header names what =C-c C-c= will do: =Edit (steer)= or =Edit (queue)=, =Edit (steer unavailable)= for an explicit steer draft the running turn cannot take, since sending will error rather than change delivery method, and =Edit (send)= when a custom =agent-shell-busy-submit-default-function= makes the outcome unknowable from here.

Explicit steering keeps the draft when the agent declines, and the buffer is read-only until the agent answers, since that draft is what gets restored.

**** Prompts that arrive as a turn ends

A steer can race the end of the turn it was aimed at.  Agent Shell asks the agent to hand the prompt back rather than start a turn of its own, but only the Claude package honors that request; Codex ignores unknown options.  When the agent starts a turn anyway, the shell says so in a block: the output arrives out of turn and the shell does not show as busy while it runs.

Agent Shell pauses automatic queue delivery for the length of a steer request, and keeps it paused when a turn started this way, so queued prompts are not fired into an agent that only looks idle.  Wait for that output to finish, then run =M-x agent-shell-prompt-queue-resume=.

=agent-shell-prompt-queue-steer= claims a pending prompt out of the queue before sending it, so turn completion cannot also submit it; a declined steer puts it back at the same position.  The prompt is therefore neither lost nor run twice.

*** Subagents and background tasks

An agent can delegate work to a subagent, or push work into the background and carry on.  Agents that report this natively get their own rendering: each background task becomes a fragment that updates in place as its state changes, rather than a tool call that reports "completed" the moment the work is handed off and then says nothing while it actually runs.

A subagent gets a read-only buffer of its own, named after the shell with =[subagent: NAME]= appended.  Its task, prompt, tool calls, thoughts and replies render there as they stream, so they never interleave with what the root session is saying.  Where the subagent was spawned, the shell shows one folded row: its state, its name and a summary of what it has done so far.  Unfold the row for its task and a button that opens its buffer.  A subagent spawned by another subagent gets its row in the spawner's buffer and a buffer of its own.  Subagent buffers are made at spawn, stay out of sight until opened, and are killed with the shell.

A subagent usually outlives the turn that spawned it.  Its row stays on the page of that turn and keeps updating there, while its work keeps going to its buffer, so nothing it does after the turn ends reads as the answer to a later prompt.

The header counts the subagents still running.  =M-x agent-shell-subagents=, or =S= in the viewport, lists every subagent and background task of the session, running ones first and finished ones under a folded group.  In the list, =o= opens a subagent's buffer, =j= jumps to its row where it was spawned, =x= stops it, and =g= redraws.  From the viewport, =j= pages the viewport to that turn.  =x= only works for what the agent says a client may stop: a subagent whose spawn advertised =capabilities.cancel=, or a background task the agent marked stoppable.  =M-x agent-shell-subagents-switch= opens a subagent's buffer by name, and =^= in a subagent buffer returns to whoever spawned it.

A permission request from a subagent shows in the shell, headed "NAME needs approval", and in the subagent's buffer.  Answering either copy removes both.  Content from a session the agent never announced still renders in the shell, after a one-time notice naming the session.

Each subagent also gets a transcript file of its own, in a =subagents/= directory beside the shell's transcript.  The shell's transcript records each spawn and each end, with a link to the subagent's file.

=M-x agent-shell-stop-async-task= stops a running background task, asking which one when the session has more than one.  Only tasks the agent says it can stop are offered.  A task the agent hides from the transcript, because a tool call already shows it, is still offered here: whether a task draws its own card and whether it can be stopped are separate questions.

This rides claude-agent-acp's JetBrains AIR extension, which the agent gates on the client asking for it by name at initialization.  Agent Shell asks for both halves, =nativeSubagentSessions= and =asyncTasks=, and only for agents whose configuration opts in.  Other agents are told nothing about the extension and keep their existing behavior.  The draft ACP =subagent_update= notification is understood too, whichever agent sends it.

*** Forking from a message

=M-x agent-shell-fork= starts a new session from the current one, carrying its history up to the latest turn.  =M-x agent-shell-fork-at-point= stops that history right after the agent message point is on, so the new session never sees the turns you meant to leave behind.  Both work from the shell buffer and the viewport.

Put point on a rendered agent message first; the message identity Agent Shell needs is stamped on the message when it renders.

The fork point rides the same AIR extension.  An agent that never advertised it would ignore the fork point and hand back a session forked from the latest turn instead, which looks like it worked and only shows up several turns later when the agent still remembers the discarded tail, so =agent-shell-fork-at-point= refuses rather than forking the wrong thing.  Use =agent-shell-fork= when the latest turn is what you want.

*** Per-model token usage

A session that runs subagents can spend on more than one model: the root session on one, its subagents on another.  When an agent reports a per-model breakdown and more than one model contributed, the usage display gains a =Models:= row splitting the turn's tokens by model.  Sessions on a single model are unchanged.

See =agent-shell-show-usage-at-turn-end= for showing usage as each turn ends.
...
|                                                       | agent-shell-fork-at-point                               | Fork the current session up to the agent message at point.                    |
...
|                                                       | agent-shell-prompt-queue-dwim                           | Queue a prompt with DWIM context for an existing shell.                       |
|                                                       | agent-shell-prompt-queue-edit                           | Edit the pending prompt at INDEX, replacing it in place.                      |
...
|                                                       | agent-shell-prompt-queue-steer                          | Steer the pending prompt at INDEX into the turn already running.              |
|                                                       | agent-shell-prompt-send                                 | Send PROMPT, steering or queueing it when a turn is already running.          |
|                                                       | agent-shell-prompt-send-dwim                            | Send a prompt with DWIM context to an existing shell.                         |
|                                                       | agent-shell-prompt-steer                                | Steer PROMPT into the turn the agent is currently running.                    |
|                                                       | agent-shell-prompt-steer-dwim                           | Steer a prompt with DWIM context into the turn already running.               |
...
|                                                       | agent-shell-stop-async-task                             | Stop a running, stoppable background async task in this session.              |
...
| C-c C-q                                               | agent-shell-viewport-compose-queue                      | Queue behind a running turn or submit when idle, then clear the buffer.       |
...
| C-c C-i                                               | agent-shell-viewport-compose-steer                      | Steer the composed prompt into the running turn, then clear the buffer.       |
...
|                                                       | agent-shell-viewport-prompt-queue-edit                  | Edit a pending prompt in the queue.                                           |
...
|                                                       | agent-shell-viewport-set-session-config-option          | Set a session config option.                                                  |
