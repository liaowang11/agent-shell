...
A subagent gets a collapsible pane of its own, headed by its name, its state and a summary of what it has done.  Its task, tool calls, thoughts and replies render inside that pane instead of merging into the root session's transcript, so folding the header puts the whole subagent away.  A subagent spawned by another subagent shares its ancestor's pane, and its content carries its name to tell the two apart.

A subagent usually outlives the turn that spawned it, and its pane stays where that turn is: work arriving after the turn ends, or while you are already several prompts further on, still lands on the page it belongs to rather than reading as the answer to a question the subagent never saw.  The pane folds when the subagent reports that it is done.
...
This rides claude-agent-acp's JetBrains AIR extension, which the agent gates on the client asking for it by name at initialization.  Agent Shell asks for both halves, =nativeSubagentSessions= and =asyncTasks=, and only for agents whose configuration opts in.  Other agents are told nothing about the extension and keep their existing behavior.
