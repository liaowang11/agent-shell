...
- =M-x agent-shell-prompt-send= - follow =agent-shell-prompt-while-busy= (see below).
...
=agent-shell-prompt-while-busy= decides what =agent-shell-prompt-send= and the compose buffer's send key do while the agent works.  It defaults to =steer=; set it to =queue= to wait for the turn to end.  A plain send stays lossless whichever way it is set: with no turn running the prompt is submitted, and an agent that cannot take a steered prompt, one that declines it, or a shell waiting on a permission answer all get the prompt queued instead.
...
The compose buffer's keys line up with those three commands: =C-c C-c= (=agent-shell-viewport-compose-send=) follows the setting, =C-c C-q= (=agent-shell-viewport-compose-queue=) queues behind a running turn or submits when idle, and =C-c C-i= (=agent-shell-viewport-compose-steer=) steers.  All three leave the compose buffer the same way after delivery, following =agent-shell-viewport-dismiss-on-send= and =agent-shell-prefer-viewport-interaction=; with a prefix argument (=C-u=) the buffer stays open, cleared, for the next prompt.

A compose buffer carries the disposition of whichever command opened it, and that outranks the setting.  A draft started by =agent-shell-prompt-queue-dwim= still queues while the setting says steer, because a draft outlives the command that started it.  While a turn is running, the edit-mode header names what =C-c C-c= will do: =Edit (steer)= or =Edit (queue)=, and =Edit (steer unavailable)= for an explicit steer draft the running turn cannot take, since sending will error rather than change delivery method.
...
An agent can delegate work to a subagent, or push work into the background and carry on.  Agents that report this natively get their own rendering: each subagent and each background task becomes a fragment that updates in place as its state changes, rather than a tool call that reports "completed" the moment the work is handed off and then says nothing while it actually runs.

Content a subagent produces is tagged with that subagent's name, so its tool calls and replies read as its own rather than merging into the root session's transcript.  A subagent's work still renders inline in the same transcript; a nested, collapsible pane per subagent is a possible follow-up rather than what this does.
...
| agent-shell-prompt-while-busy                 | What ‘agent-shell-prompt-send’ does while the agent is working on a turn.       |
